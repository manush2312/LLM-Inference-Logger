...
| **`message_id` is nullable** | Errors and cancellations produce no assistant message at all — precisely the rows the dashboard exists to surface — so `NOT NULL` would make the most interesting logs unwritable. In practice it is null on *every* row, including successes, for a second reason: publishing the id would race the worker into a foreign-key violation. See [Known limitations](#known-limitations) |
